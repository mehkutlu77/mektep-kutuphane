...
// Tam sayfa kapak görselleri (ikisi de `web sayfası/` altında, teslim edilen
...
const ON_KAPAK = 'web sayfası/kapak.png';
const ARKA_KAPAK = 'web sayfası/arkakapak.png';
...
async function main() {
    console.log("PDFKit ile E-Kitapçıklar Üretiliyor...");
    
    let rawAuthors = [];
    try {
        const rawData = fs.readFileSync('yazarlar_veritabani.json', 'utf8');
    ...
    if (!fs.existsSync('web sayfası/flipbook/data')) {
        fs.mkdirSync('web sayfası/flipbook/data', { recursive: true });
    }
    if (!fs.existsSync('web sayfası/pdf_ciktilari')) {
        fs.mkdirSync('web sayfası/pdf_ciktilari', { recursive: true });
    }
    ...
    for (let i = 0; i < authors.length; i++) {
        ...
        const outputPath = `web sayfası/flipbook/data/${safeName}.js`;
        fs.writeFileSync(outputPath, jsIcerik);

        // Save raw .pdf file to web sayfası/pdf_ciktilari/
        const pdfPath = `web sayfası/pdf_ciktilari/${safeName}.pdf`;
    ...
    fs.writeFileSync('yazarlar_veritabani.json', JSON.stringify(rawAuthors, null, 2), 'utf8');
