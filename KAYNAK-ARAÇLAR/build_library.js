...
<body>
    ...
    <script>
        // Oturum kontrolü (Sadece yönetici)
        const role = localStorage.getItem('userRole');
        if (role !== 'admin') {
            window.location.href = 'login.html';
        }
        ...
        async function loadPendingCount() {
            try {
                ...
                if (data.success && data.pendingArticles) {
                    const count = data.pendingArticles.length;
...
function build() {
    ...
    const authors = JSON.parse(fs.readFileSync('yazarlar_veritabani.json', 'utf-8'));
    
    const libraryHtml = createLibraryHTML(authors);
    fs.writeFileSync('web sayfası/mektep_kutuphane.html', libraryHtml);
    console.log("✓ Kütüphane arayüzü (web sayfası/mektep_kutuphane.html) oluşturuldu.");

    const adminHtml = createAdminHTML(authors);
    fs.writeFileSync('web sayfası/mektep_yonetim.html', adminHtml);
    console.log("✓ Yönetici & İstatistik paneli (web sayfası/mektep_yonetim.html) oluşturuldu.");
    ...
    fs.writeFileSync(
        'web sayfası/yazar_kullanicilari.js',
    ...
    console.log(`✓ Yazar kullanıcı adı listesi (web sayfası/yazar_kullanicilari.js) — ${Object.keys(kullanicilar).length} kayıt.`);
}
